...
func createSecret(secret *corev1.Secret, name string, namespace string) (*corev1.Secret, error) {
	labels := map[string]string{
		"secretsync.verax.net/replicated-from": fmt.Sprintf("%s.%s", secret.Namespace, secret.Name),
	}
	annotations := secret.Annotations
	annotations["secretsync.verax.net/replicated-time"] = time.Now().Format("Mon Jan 2 15:04:05 MST 2006")
	annotations["secretsync.verax.net/replicated-resource-version"] = secret.ResourceVersion
...
func (r *ReconcileSecret) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	//cert-manager.io/certificate-name secretsync.verax.net/replicated-time
	// check is it generated from cert-manager
	if _, ok := instance.Annotations["cert-manager.io/certificate-name"]; ok {
		// only "original"
		if _, ok := instance.Annotations["secretsync.verax.net/replicated-time"]; !ok {
			reqLogger.Info(fmt.Sprintf("Found cert manager secret [%s] in the [%s].", instance.Name, instance.Namespace))
			namespaceType := &corev1.NamespaceList{}
			err := r.client.List(context.TODO(), namespaceType)
			if err != nil {
				return reconcile.Result{}, err
			}
			...
			for _, namespace := range namespaces {
				if namespace != instance.Namespace {
					targetSecret, err := createSecret(instance, instance.Name, namespace)
					if err != nil {
						return reconcile.Result{}, err
					}
					...
					if err != nil && errors.IsNotFound(err) {
						reqLogger.Info(fmt.Sprintf("Target secret %s/%s doesn't exist, creating it", namespace, instance.Name))
						err = r.client.Create(context.TODO(), targetSecret)
						if err != nil {
							return reconcile.Result{}, err
						}
					} else {
						reqLogger.Info(fmt.Sprintf("Target secret %s/%s exists, updating it now", namespace, instance.Name))
			...
			reqLogger.Info(fmt.Sprintf("Found replica  [%s] in the [%s].", instance.Name, instance.Namespace))
		}

	}
	if rep, ok := instance.Annotations["secretsync.verax.net/replicate"]; ok {
		if rep == "true" {
			if tgts, ok := instance.Annotations["secretsync.verax.net/replicate-to"]; ok {
				namespaceType := &corev1.NamespaceList{}
				err := r.client.List(context.TODO(), namespaceType)
				if err != nil {
					return reconcile.Result{}, err
